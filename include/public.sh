#!/usr/bin/env bash
# Copyright (C) 2022 SLMP <hi@slmp.sh>
# 
# This file is part of the SLMP script.
#
# SLMP is a simple bash script for the installation of 
...
# Description:  Install SLMP(Simple Linux + Apache + MySQL/MariaDB + PHP )
# Website:  https://slmp.sh
...
display_os_info(){
    clear
    echo
    echo "    _____  __     __  ___ ____  "
    echo "   / ___/ / /    /  |/  // __ \ "
    echo "   \__ \ / /    / /|_/ // /_/ / "
    echo "  ___/ // /___ / /  / // ____/  "
    echo " /____//_____//_/  /_//_/       "
    echo "                                        "
    echo " Simple Linux + Apache + MySQL/MariaDB + PHP "
    echo " Made by SLMP - https://slmp.sh "
    echo " contact: hi@slmp.sh "
    echo
...
EOF
        ...
        echo "The Full Log is available at ${cur_dir}/slmp.log"
        echo "Please visit website: https://slmp.sh/faq for help"
        exit 1
    fi
}
...
install_finally(){
    ...
    echo "---------------------------------------------------------------------"
    echo
    cp -f ${cur_dir}/conf/slmp /usr/bin/slmp
    chmod +x /usr/bin/slmp
    sed -i "s@^apache_location=.*@apache_location=${apache_location}@" /usr/bin/slmp
    sed -i "s@^mysql_location=.*@mysql_location=${mysql_location}@" /usr/bin/slmp
    sed -i "s@^mariadb_location=.*@mariadb_location=${mariadb_location}@" /usr/bin/slmp
    sed -i "s@^web_root_dir=.*@web_root_dir=${web_root_dir}@" /usr/bin/slmp
    ldconfig
...
EOF
    ...
    _info "Welcome to visit our website: https://slmp.sh"
    _info "Enjoy it"
    exit 0
}
