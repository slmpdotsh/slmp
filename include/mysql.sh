...
mysql_preinstall_settings(){
    ...
    if [ "${mysql}" != "do_not_install" ];then
        if echo "${mysql}" | grep -qi "mysql"; then
            ...
            echo
            read -p "mysql server root password (default:slmp.sh, leave blank for default): " mysql_root_pass
            mysql_root_pass=${mysql_root_pass:=slmp.sh}
            echo
            ...
            echo
            read -p "mariadb server root password (default:slmp.sh, leave blank for default): " mariadb_root_pass
            mariadb_root_pass=${mariadb_root_pass:=slmp.sh}
            echo
